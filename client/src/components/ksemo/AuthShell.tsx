...
export function AuthError({ message }: { message?: string | null }) {
  ...
  return (
    <p role="alert" className="rounded-xl border border-destructive/30 bg-destructive/10 px-3 py-2 text-xs leading-4 text-destructive">
      {message}
    </p>
  );
}
...
type TextFieldProps = FieldProps & Omit<React.ComponentProps<typeof Input>, "id">;

export function AuthTextField({ label, error, action, ...inputProps }: TextFieldProps) {
  const id = useId();
  return (
    <FieldFrame label={label} error={error} action={action}>
      <Input id={id} autoComplete="off" className="h-11 rounded-xl border-border bg-background/60 text-[15px] shadow-none" {...inputProps} />
    </FieldFrame>
  );
}

type PasswordFieldProps = FieldProps & Omit<React.ComponentProps<typeof Input>, "id" | "type"> & { autoComplete?: string };

export function AuthPasswordField({ label, error, action, ...inputProps }: PasswordFieldProps) {
  const id = useId();
...
export function AuthShell({ eyebrow = "KSEMO", title, subtitle, children, footer }: AuthShellProps) {
  return (
    <main className="relative grid h-dvh place-items-center overflow-hidden bg-background px-5 py-6 sm:px-6">
      ...
      <section className="relative w-full max-w-sm text-center">
        <a href="/" className="mx-auto block size-11 overflow-hidden rounded-xl border border-border bg-card shadow-[0_14px_40px_rgba(0,0,0,0.16)]">
          <img src="/KSEMOlogo.png" alt="KSEMO logo" className="size-full object-cover" />
        </a>
        <p className="mt-3.5 text-[11px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">{eyebrow}</p>
        <h1 className="mt-2 text-[1.65rem] font-semibold leading-tight tracking-[-0.03em] sm:text-[1.85rem]">{title}</h1>
        <p className="mx-auto mt-2 max-w-xs text-[13px] leading-5 text-muted-foreground">{subtitle}</p>
        ...
        {footer ? <div className="mt-4 space-y-1.5 text-xs leading-5 text-muted-foreground">{footer}</div> : null}
      </section>
    </main>
  );
}
