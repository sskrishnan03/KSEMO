...
export default function Signup() {
  ...
  const submitEmail = async (e: FormEvent) => {
    ...
    if (s.score < 2) { setError('Choose a stronger password (8+ chars, mixed case, numbers).'); return; }
    setLoading(true);
    ...
    if (data.session) { nav('/app', { replace: true }); return; }
    const { error: signInError } = await supabase.auth.signInWithPassword({ email, password });
    setLoading(false);
    if (signInError) { nav('/login', { replace: true }); return; }
    nav('/app', { replace: true });
  };
  ...
  useEffect(() => {
    const gsiReady = () => {
      ...
      const el = document.getElementById('google-btn-inner-signup');
    ...
    >
      <div className="space-y-3">
        ...
        <div className="relative">
          ...
          <div id="google-btn-inner-signup" className="absolute inset-0 opacity-0 cursor-pointer" />
        </div>
