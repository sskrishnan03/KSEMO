...
export function Sidebar({ collapsed, onToggleCollapse }: Props) {
  ...
  useEffect(() => {
    if (profile?.id) getSettings(profile.id).then((s) => setPrefs(s?.preferences ?? {})).catch(() => {});
  }, [profile?.id]);
  ...
  const filtered = chats.filter((c) => {
    ...
    if (dateFilter === 'today') return diffDays < 1 && now.getDate() === d.getDate();
  ...
  const compact = prefs.compact_mode ?? false;
      ...
      <aside className={cn(
        ...
        <div className={cn(
          ...
          <nav className={cn('px-2 pt-3 pb-2 space-y-0.5 shrink-0', compact && 'space-y-0 pt-2')}>
            ...
            {navItems.map((n) => (
              <Link
                ...
                to={n.to}
                className={cn(
                  'group flex items-center gap-3 px-3 rounded-lg text-[13px] transition-colors',
              ...
              <div className="relative">
                <button
                  ...
                  className={cn(
                    ...
                    dateFilter !== 'all' ? 'bg-white/10 text-white' : 'text-ink-300 hover:text-white hover:bg-white/5',
                  )}
                ...
                {dateFilterOpen && (
                  <>
                    ...
                    <div className="absolute right-0 top-full mt-1 z-50 w-44 bg-ink-850 border border-white/10 rounded-xl shadow-lift py-1 overflow-hidden">
                      {([['all', 'All dates'], ['today', 'Today'], ['yesterday', 'Yesterday'], ['week', 'This week'], ['month', 'This month']] as const).map(([val, label]) => (
                        <button
                          key={val}
                          onClick={() => { setDateFilter(val); setCustomDate(''); setDateFilterOpen(false); }}
                          className={cn(
                            'w-full text-left px-3 py-1.5 text-[12px] transition',
                            dateFilter === val ? 'bg-white/10 text-white' : 'text-ink-200 hover:bg-white/5 hover:text-white',
                          )}
                        >
                          {label}
                        </button>
                      ...
                      <div className="px-3 py-1.5">
                        <div className="text-[11px] text-ink-400 mb-1">Pick a date</div>
                        <input
                          type="date"
                          value={customDate}
                          onChange={(e) => { setCustomDate(e.target.value); setDateFilter('custom'); }}
                          className="w-full h-7 px-2 rounded-lg bg-ink-900 border border-white/10 text-[12px] text-white focus:outline-none focus:border-white/25"
                        />
                      </div>
                      {dateFilter === 'custom' && customDate && (
                        <button
                          onClick={() => { setDateFilter('all'); setCustomDate(''); setDateFilterOpen(false); }}
          ...
          <div className="flex-1 overflow-y-auto px-2 py-2 scrollbar-hide">
            {filtered.length === 0 && <p className="px-3 py-4 text-[12px] text-ink-300 text-center">{dateFilter !== 'all' ? 'No chats in this period' : 'No chats here yet'}</p>}
            {grouped.map((g) => (
      ...
      {accountMenuOpen && (
        <>
          ...
          <div className={cn(
            ...
            <button
              onClick={() => { setAccountMenuOpen(false); nav('/app/settings?tab=help'); }}
              className="w-full flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-[12px] text-ink-100 hover:bg-white/5 hover:text-white transition text-left"
            >
